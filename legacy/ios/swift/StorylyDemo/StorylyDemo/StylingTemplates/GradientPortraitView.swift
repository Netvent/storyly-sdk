...
public class GradientPortraitView: StoryGroupView {
    
    let kCONTENT_XIB_NAME = "GradientPortraitView"
    ...
    func commonInit() {
        ...
        if let image = UIImage(named: "st_pin_icon", in: Bundle(for: StorylyView.self as AnyClass), compatibleWith: nil)?.imageWithInsets(insetDimen: 8) {
            pinIcon.setImage(image: image, contentMode: .scaleAspectFit)
        }
        if let image = UIImage(named: "st_ivod_icon", in: Bundle(for: StorylyView.self as AnyClass), compatibleWith: nil)?.imageWithInsets(insetDimen: 8) {
...
public class GradientPortraitViewFactory: StoryGroupViewFactory {
    public func getSize() -> CGSize {
        return CGSize(width: 100, height: 178)
    }
    
    public func createView() -> StoryGroupView {
        return GradientPortraitView(frame: .zero)
    }
}
