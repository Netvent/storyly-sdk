...
#import "NSData+ST_ImageContentType.h"

typedef NSString * ST_SDImageCoderOption NS_STRING_ENUM;
typedef NSDictionary<ST_SDImageCoderOption, id> ST_SDImageCoderOptions;
typedef NSMutableDictionary<ST_SDImageCoderOption, id> ST_SDImageCoderMutableOptions;
...
/**
 ...
 @note works for `ST_SDImageCoder`.
 */
FOUNDATION_EXPORT ST_SDImageCoderOption _Nonnull const ST_SDImageCoderDecodeFirstFrameOnly;
...
/**
 ...
 @note works for `ST_SDImageCoder`, `ST_SDProgressiveImageCoder`, `ST_SDAnimatedImageCoder`.
 */
FOUNDATION_EXPORT ST_SDImageCoderOption _Nonnull const ST_SDImageCoderDecodeScaleFactor;
...
/**
 ...
 @note works for `ST_SDImageCoder`, `ST_SDProgressiveImageCoder`, `ST_SDAnimatedImageCoder`.
 */
FOUNDATION_EXPORT ST_SDImageCoderOption _Nonnull const ST_SDImageCoderDecodePreserveAspectRatio;
...
/**
 ...
 @note works for `ST_SDImageCoder`, `ST_SDProgressiveImageCoder`, `ST_SDAnimatedImageCoder`.
 */
FOUNDATION_EXPORT ST_SDImageCoderOption _Nonnull const ST_SDImageCoderDecodeThumbnailPixelSize;
...
/**
 ...
 @note works for `ST_SDImageCoder`.
 */
FOUNDATION_EXPORT ST_SDImageCoderOption _Nonnull const ST_SDImageCoderEncodeFirstFrameOnly;
/**
 ...
 @note works for `ST_SDImageCoder`
 */
FOUNDATION_EXPORT ST_SDImageCoderOption _Nonnull const ST_SDImageCoderEncodeCompressionQuality;
...
/**
 ...
 @note works for `ST_SDImageCoder`
 */
FOUNDATION_EXPORT ST_SDImageCoderOption _Nonnull const ST_SDImageCoderEncodeBackgroundColor;
...
/**
 ...
 @note works for `ST_SDImageCoder`
 */
FOUNDATION_EXPORT ST_SDImageCoderOption _Nonnull const ST_SDImageCoderEncodeMaxFileSize;
...
/**
 ...
 @note works for `ST_SDImageCoder`
 */
FOUNDATION_EXPORT ST_SDImageCoderOption _Nonnull const ST_SDImageCoderEncodeEmbedThumbnail;

/**
 A ST_SDWebImageContext object which hold the original context options from top-level API. (ST_SDWebImageContext)
 ...
 See `ST_SDWebImageContext` for more detailed information.
 */
FOUNDATION_EXPORT ST_SDImageCoderOption _Nonnull const ST_SDImageCoderWebImageContext API_DEPRECATED("The coder component will be seperated from Core subspec in the future. Update your code to not rely on this context option.", macos(10.10, API_TO_BE_DEPRECATED), ios(8.0, API_TO_BE_DEPRECATED), tvos(9.0, API_TO_BE_DEPRECATED), watchos(2.0, API_TO_BE_DEPRECATED));
 ...
 */
@protocol ST_SDImageCoder <NSObject>
...
/**
 ...
 @param options A dictionary containing any decoding options. Pass @{ST_SDImageCoderDecodeScaleFactor: @(1.0)} to specify scale factor for image. Pass @{ST_SDImageCoderDecodeFirstFrameOnly: @(YES)} to decode the first frame only.
 @return The decoded image from data
 */
- (nullable UIImage *)decodedImageWithData:(nullable NSData *)data
                                   options:(nullable ST_SDImageCoderOptions *)options;

#pragma mark - Encoding

/**
 Returns YES if this coder can encode some image. Otherwise, it should be passed to another coder.
 For custom coder which introduce new image format, you'd better define a new `ST_SDImageFormat` using like this. If you're creating public coder plugin for new image format, also update `https://github.com/rs/SDWebImage/wiki/Coder-Plugin-List` to avoid same value been defined twice.
 * @code
 static const ST_SDImageFormat ST_SDImageFormatHEIF = 10;
 * @endcode
 
 @param format The image format
 @return YES if this coder can encode the image, NO otherwise
 */
- (BOOL)canEncodeToFormat:(ST_SDImageFormat)format NS_SWIFT_NAME(canEncode(to:));
...
/**
 ...
 @param format The image format to encode, you should note `ST_SDImageFormatUndefined` format is also  possible
 @param options A dictionary containing any encoding options. Pass @{ST_SDImageCoderEncodeCompressionQuality: @(1)} to specify compression quality.
 @return The encoded image data
 */
- (nullable NSData *)encodedDataWithImage:(nullable UIImage *)image
                                   format:(ST_SDImageFormat)format
                                  options:(nullable ST_SDImageCoderOptions *)options;
...
/**
 ...
 @note Pay attention that these methods are not called from main queue.
 */
@protocol ST_SDProgressiveImageCoder <ST_SDImageCoder>
...
/**
 ...
 @param options A dictionary containing any progressive decoding options (instance-level). Pass @{ST_SDImageCoderDecodeScaleFactor: @(1.0)} to specify scale factor for progressive animated image (each frames should use the same scale).
 @return A new instance to do incremental decoding for the specify image format
 */
- (nonnull instancetype)initIncrementalWithOptions:(nullable ST_SDImageCoderOptions *)options;
...
/**
 ...
 @note Due to the performance issue for progressive decoding and the integration for image view. This method may only return the first frame image even if the image data is animated image. If you want progressive animated image decoding, conform to `ST_SDAnimatedImageCoder` protocol as well and use `animatedImageFrameAtIndex:` instead.

 @param options A dictionary containing any progressive decoding options. Pass @{ST_SDImageCoderDecodeScaleFactor: @(1.0)} to specify scale factor for progressive image
 @return The decoded image from current data
 */
- (nullable UIImage *)incrementalDecodedImageWithOptions:(nullable ST_SDImageCoderOptions *)options;

@end

#pragma mark - Animated Image Provider
/**
 This is the animated image protocol to provide the basic function for animated image rendering. It's adopted by `SDAnimatedImage` and `ST_SDAnimatedImageCoder`
 */
@protocol ST_SDAnimatedImageProvider <NSObject>
...
/**
 This is the animated image coder protocol for custom animated image class like  `SDAnimatedImage`. Through it inherit from `ST_SDImageCoder`. We currentlly only use the method `canDecodeFromData:` to detect the proper coder for specify animated image format.
 */
@protocol ST_SDAnimatedImageCoder <ST_SDImageCoder, ST_SDAnimatedImageProvider>
...
/**
 ...
 After the instance created, we may call methods in `ST_SDAnimatedImageProvider` to produce animated image frame.

 @param data The animated image data to be decode
 @param options A dictionary containing any animated decoding options (instance-level). Pass @{ST_SDImageCoderDecodeScaleFactor: @(1.0)} to specify scale factor for animated image (each frames should use the same scale).
 @return A new instance to do animated decoding for specify image data
 */
- (nullable instancetype)initWithAnimatedImageData:(nullable NSData *)data options:(nullable ST_SDImageCoderOptions *)options;
