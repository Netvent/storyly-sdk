...
typedef void(^ST_SDImageLoaderProgressBlock)(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL);
typedef void(^ST_SDImageLoaderCompletedBlock)(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished);

#pragma mark - Context Options

/**
 A `UIImage` instance from `SDWebImageManager` when you specify `ST_SDWebImageRefreshCached` and image cache hit.
 This can be a hint for image loader to load the image from network and refresh the image from remote location if needed. If the image from remote location does not change, you should call the completion with `ST_SDWebImageErrorCacheNotModified` error. (UIImage)
 @note If you don't implement `ST_SDWebImageRefreshCached` support, you do not need to care about this context option.
 */
FOUNDATION_EXPORT ST_SDWebImageContextOption _Nonnull const ST_SDWebImageContextLoaderCachedImage;
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable ST_SDImageLoaderDecodeImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, ST_SDWebImageOptions options, ST_SDWebImageContext * _Nullable context);

/**
 This is the built-in decoding process for image progressive download from network. It's used when `ST_SDWebImageProgressiveLoad` option is set. (It's not required when your loader does not support progressive image loading)
 @note If you want to implement your custom loader with `requestImageWithURL:options:context:progress:completed:` API, but also want to keep compatible with SDWebImage's behavior, you'd better use this to produce image.
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable ST_SDImageLoaderDecodeProgressiveImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, BOOL finished,  id<ST_SDWebImageOperation> _Nonnull operation, ST_SDWebImageOptions options, ST_SDWebImageContext * _Nullable context);
 ...
 */
FOUNDATION_EXPORT id<ST_SDProgressiveImageCoder> _Nullable ST_SDImageLoaderGetProgressiveCoder(id<ST_SDWebImageOperation> _Nonnull operation);
 ...
 */
FOUNDATION_EXPORT void ST_SDImageLoaderSetProgressiveCoder(id<ST_SDWebImageOperation> _Nonnull operation, id<ST_SDProgressiveImageCoder> _Nullable progressiveCoder);

#pragma mark - ST_SDImageLoader
...
/**
 ...
 If you want to implement custom loader for image download from network or local file, you just need to concentrate on image data download only. After the download finish, call `ST_SDImageLoaderDecodeImageData` or `ST_SDImageLoaderDecodeProgressiveImageData` to use the built-in decoding process and produce image (Remember to call in the global queue). And finally callback the completion block.
...
*/
@protocol ST_SDImageLoader <NSObject>
 ...
 */
- (nullable id<ST_SDWebImageOperation>)requestImageWithURL:(nullable NSURL *)url
                                                options:(ST_SDWebImageOptions)options
                                                context:(nullable ST_SDWebImageContext *)context
                                               progress:(nullable ST_SDImageLoaderProgressBlock)progressBlock
                                              completed:(nullable ST_SDImageLoaderCompletedBlock)completedBlock;


/**
 Whether the error from image loader should be marked indeed un-recoverable or not.
 If this return YES, failed URL which does not using `ST_SDWebImageRetryFailed` will be blocked into black list. Else not.
