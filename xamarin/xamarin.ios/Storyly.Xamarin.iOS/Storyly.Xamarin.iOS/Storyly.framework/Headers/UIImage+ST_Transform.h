...
typedef NS_ENUM(NSUInteger, ST_SDImageScaleMode) {
    ST_SDImageScaleModeFill = 0,
    ST_SDImageScaleModeAspectFit = 1,
    ST_SDImageScaleModeAspectFill = 2
};

#if SD_UIKIT || SD_WATCH
typedef UIRectCorner ST_SDRectCorner;
#else
typedef NS_OPTIONS(NSUInteger, ST_SDRectCorner) {
    ST_SDRectCornerTopLeft     = 1 << 0,
    ST_SDRectCornerTopRight    = 1 << 1,
    ST_SDRectCornerBottomLeft  = 1 << 2,
    ST_SDRectCornerBottomRight = 1 << 3,
    ST_SDRectCornerAllCorners  = ~0UL
};
#endif
 ...
 */
@interface UIImage (ST_Transform)
 ...
 */
- (nullable UIImage *)st_resizedImageWithSize:(CGSize)size scaleMode:(ST_SDImageScaleMode)scaleMode;
 ...
 */
- (nullable UIImage *)st_croppedImageWithRect:(CGRect)rect;
 ...
 */
- (nullable UIImage *)st_roundedCornerImageWithRadius:(CGFloat)cornerRadius
                                              corners:(ST_SDRectCorner)corners
 ...
 */
- (nullable UIImage *)st_rotatedImageWithAngle:(CGFloat)angle fitSize:(BOOL)fitSize;
 ...
 */
- (nullable UIImage *)st_flippedImageWithHorizontal:(BOOL)horizontal vertical:(BOOL)vertical;
 ...
 */
- (nullable UIImage *)st_tintedImageWithColor:(nonnull UIColor *)tintColor;
 ...
 */
- (nullable UIColor *)st_colorAtPoint:(CGPoint)point;
...
/**
 ...
 @note The rect's width/height should not be smaller than or equal to 0. The minX/minY should not be smaller than 0. The maxX/maxY should not be greater than width/height. Attention this limit is different from `st_colorAtPoint:` (point: (0, 0) like rect: (0, 0, 1, 1))
 @note The overhead of object creation means this method is best suited for infrequent color sampling. For heavy image processing, grab the raw bitmap data and process yourself.

 @param rect The rectangle of pixels
 @return The color array for specify pixels, or nil if any error occur
 */
- (nullable NSArray<UIColor *> *)st_colorsWithRect:(CGRect)rect;
 ...
 */
- (nullable UIImage *)st_blurredImageWithRadius:(CGFloat)blurRadius;
...
/**
 ...
 enough memory).
 */
- (nullable UIImage *)st_filteredImageWithFilter:(nonnull CIFilter *)filter;
#endif
