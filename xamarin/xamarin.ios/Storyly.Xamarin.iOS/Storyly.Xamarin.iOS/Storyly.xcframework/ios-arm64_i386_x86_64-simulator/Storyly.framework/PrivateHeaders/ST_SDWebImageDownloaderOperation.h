 ...
 * For example, if we config [200, 400) but server response is 503, the download will fail with error code `ST_SDWebImageErrorInvalidDownloadStatusCode`.
 ...
 * For example, if we config ["image/png"] but server response is "application/json", the download will fail with error code `ST_SDWebImageErrorInvalidDownloadContentType`.
 ...
 */
@property (copy, nonatomic, readonly, nullable) ST_SDWebImageContext *context;
...
- (nonnull instancetype)initWithRequest:(nullable NSURLRequest *)request
                              inSession:(nullable NSURLSession *)session
                                options:(SDWebImageDownloaderOptions)options
                                context:(nullable ST_SDWebImageContext *)context NS_DESIGNATED_INITIALIZER;
