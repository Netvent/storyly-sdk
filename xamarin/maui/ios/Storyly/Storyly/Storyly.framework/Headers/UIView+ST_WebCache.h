 ...
 */
FOUNDATION_EXPORT const int64_t ST_SDWebImageProgressUnitCountUnknown; /* 1LL */

typedef void(^ST_SDSetImageBlock)(UIImage * _Nullable image, NSData * _Nullable imageData, ST_SDImageCacheType cacheType, NSURL * _Nullable imageURL);
 ...
 */
@property (nonatomic, strong, readonly, nullable) NSURL *st_imageURL;
 ...
 * See more about this in `ST_SDWebImageContextSetImageOperationKey`.
 * If you cancel current image load, the key will be set to nil.
 * @note You can use method `UIView+ST_WebCacheOperation` to investigate different queries' operation.
 */
@property (nonatomic, strong, readonly, nullable) NSString *st_latestOperationKey;
 ...
 * The `totalUnitCount` and `completedUnitCount` will be reset to 0 after a new image loading start (change from current queue). And they will be set to `ST_SDWebImageProgressUnitCountUnknown` if the progressBlock not been called but the image loading success to mark the progress finished (change from main queue).
 ...
 */
@property (nonatomic, strong, null_resettable) NSProgress *st_imageProgress;
 ...
 *
 *   The fifth parameter normally is always YES. However, if you provide ST_SDWebImageAvoidAutoSetImage with ST_SDWebImageProgressiveLoad options to enable progressive downloading and set the image yourself. This block is thus called repeatedly with a partial image. When image is fully downloaded, the
 *   block is called a last time with the full image and the last parameter set to YES.
 *
 *   The last parameter is the original image URL
 */
- (void)st_internalSetImageWithURL:(nullable NSURL *)url
                  placeholderImage:(nullable UIImage *)placeholder
                           options:(ST_SDWebImageOptions)options
                           context:(nullable ST_SDWebImageContext *)context
                     setImageBlock:(nullable ST_SDSetImageBlock)setImageBlock
                          progress:(nullable ST_SDImageLoaderProgressBlock)progressBlock
                         completed:(nullable ST_SDInternalCompletionBlock)completedBlock;

/**
 * Cancel the current image load
 */
- (void)st_cancelCurrentImageLoad;
 ...
 */
@property (nonatomic, strong, nullable) ST_SDWebImageTransition *st_imageTransition;
 ...
 */
@property (nonatomic, strong, nullable) id<ST_SDWebImageIndicator> st_imageIndicator;
