...
/**
 Gets the default cache config used for shared instance or initialization when it does not provide any cache config. Such as `ST_SDImageCache.sharedImageCache`.
...
/*
 * The option to control weak memory cache for images. When enable, `ST_SDImageCache`'s memory cache will use a weak maptable to store the image at the same time when it stored to memory, and get removed at the same time.
 ...
 */
@property (assign, nonatomic) ST_SDImageCacheConfigExpireType diskCacheExpireType;
