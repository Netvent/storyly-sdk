...
public class LandscapeView: StoryGroupView {
    
    let kCONTENT_XIB_NAME = "LandscapeView"
    ...
    func commonInit() {
        ...
        if let image = UIImage(named: "st_pin_icon", in: Bundle(for: StorylyView.self as AnyClass), compatibleWith: nil)?.imageWithInsets(insetDimen: 6) {
            pinIcon.setImage(image: image, contentMode: .scaleAspectFit)
        }
        if let image = UIImage(named: "st_ivod_icon", in: Bundle(for: StorylyView.self as AnyClass), compatibleWith: nil)?.imageWithInsets(insetDimen: 6) {
...
public class LandscapeViewFactory: StoryGroupViewFactory {
    public func getSize() -> CGSize {
        return CGSize(width: 178, height: 50)
    }
    
    public func createView() -> StoryGroupView {
        return LandscapeView(frame: .zero)
    }
}
