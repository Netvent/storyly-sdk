...
import com.appsamurai.storylydemo.databinding.StylingGradientPortraitBinding
import com.appsamurai.storylydemo.styling_templates.ui.dpToPixel
import com.bumptech.glide.Glide

@SuppressLint("ViewConstructor")
class GradientPortraitView(context: Context) : StoryGroupView(context) {

    private val listViewBinding: StylingGradientPortraitBinding = StylingGradientPortraitBinding.inflate(LayoutInflater.from(context))
    ...
    override fun populateView(storyGroup: StoryGroup?) {
        ...
        if (storyGroup?.type == StoryGroupType.Vod) {
            ...
            listViewBinding.vodIcon.background = GradientDrawable().apply {
                colors = listOf(Color.parseColor("#7A4BFF"), Color.parseColor("#7A4BFF")).toIntArray()
            ...
            listViewBinding.pinIcon.avatarBackgroundColor = Color.parseColor("#FFCB00")
        }
...
class GradientPortraitViewFactory(val context: Context): StoryGroupViewFactory() {
    override fun createView(): StoryGroupView {
        return GradientPortraitView(context)
    }
}