package com.appsamurai.storylydemo.styling_templates.ui
...
class RoundImageView @JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0, private var cornerRadius: Float = dpToPixel(40.0F), private var iconBackgroundColor: Int = Color.TRANSPARENT
     ...
     */
    var borderColor by Delegates.observable(arrayOf(Color.TRANSPARENT, Color.TRANSPARENT)) { _, _, _ ->
        distanceToBorder = dpToPixel(2f).roundToInt()
        borderThickness = dpToPixel(3f).roundToInt()
        setup()
    }
    /**
     ...
     */
    internal var avatarBackgroundColor by Delegates.observable(iconBackgroundColor) { _, _, _ ->
        setup()
    }
    ...
    init {
        context.theme.obtainStyledAttributes(attrs, R.styleable.RoundImageView, 0, 0).apply {
            avatarBackgroundColor = getColor(R.styleable.RoundImageView_backgroundColor, iconBackgroundColor)
            cornerRadius = getDimension(R.styleable.RoundImageView_cornerRadius, cornerRadius)
        }
    ...
    private fun setup() {
        ...
        isRect = cornerRadius < min(width,height) / 2
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (isRect) {
            val avatarCornerRadius = max(cornerRadius - avatarInset, 0.0f)
            val middleCornerRadius = max(cornerRadius - (borderThickness + middleThickness / 2), 0.0f)
    ...
    private fun drawBorder(canvas: Canvas) {
        if (isAnimating || isReversedAnimating) {
            ...
            if (isRect) {
                val cornerRadius = max(cornerRadius - borderThickness / 2, 0.0f)
...
fun dpToPixel(dpValue: Float): Float {
